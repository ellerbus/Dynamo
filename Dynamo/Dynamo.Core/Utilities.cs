...
namespace Dynamo.Core
{
    ...
    {
        internal static string GetRelativePath(string start, string destination)
        {
            Uri startUri = new Uri(Path.GetDirectoryName(start) + Path.DirectorySeparatorChar);

            Uri destUri = new Uri(destination);

            string relativePath = startUri.MakeRelativeUri(destUri).ToString();
